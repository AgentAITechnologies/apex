...
class PrintUIMessage_Callback(StateCallback):
    def on_enter(self, csm, locals) -> None:
        dprint(f"{self.PRINT_PREFIX} Entering PrintUIMessage")
        # Perform actions when entering PrintUIMessage
        pass

    def on_exit(self, csm, locals) -> None:
        dprint(f"{self.PRINT_PREFIX} Exiting PrintUIMessage")
        # Perform actions when exiting PrintUIMessage
        pass

class AssignAction_Callback(StateCallback):
    def on_enter(self, csm, locals) -> None:
        dprint(f"{self.PRINT_PREFIX} Entering AssignAction")
...
class Exit_Callback(StateCallback):
    def on_enter(self, csm, locals) -> None:
        dprint(f"{self.PRINT_PREFIX} Entering Exit")
        # Perform actions when entering Exit
        pass

    def on_exit(self, csm, locals) -> None:
        dprint(f"{self.PRINT_PREFIX} Exiting Exit")
        # Perform actions when exiting Exit
        pass

class Start_Callback(StateCallback):
    def on_enter(self, csm, locals) -> None:
        dprint(f"{self.PRINT_PREFIX} Entering Start")
