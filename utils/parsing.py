...
def sanitize_xml(xml_string: str) -> str:
    """
    Sanitizes raw XML strings generated by LLMs by fixing bare ampersands
    and escaping raw angle brackets inside text elements that aren't valid XML.
    """
    # Fix bare ampersands that are not part of valid XML entities
    xml_string = re.sub(r'&(?!(amp|lt|gt|apos|quot|#[0-9]+|#x[0-9a-fA-F]+);)', '&amp;', xml_string)
    ...
    def fix_tag_content(match):
        ...
        # Do not alter root tag content directly as escaping it destroys the entire XML element tree
        if tag_name == "root":
            return match.group(0)

        # Check if the inner content is valid XML on its own
        try:
            ET.fromstring(f"<test>{content}</test>")
            return match.group(0)  # Valid nested XML, preserve as-is
        except ET.ParseError:
            # Content contains invalid XML/raw angle brackets; escape < and > inside
            full_escaped = content.replace("<", "&lt;").replace(">", "&gt;")
            return f"<{tag_name}{attributes}>{full_escaped}</{tag_name}>"

    # Match innermost tag pairs <tag>content</tag>
    ...
    while current != previous:
        previous = current
        current = re.sub(pattern, fix_tag_content, current, flags=re.DOTALL)
...
def xmlstr2dict(xml_string: str, client: Anthropic, depth: int = 0) -> dict:
    def escape_code_blocks(text: str) -> str:
        ...
        # Match code fences even if extra tags exist on the header line
    ...
    try:
        ...
        if depth < 6:
            ...
            system_prompt = """You are an expert in the field of programming, and are especially good at finding mistakes in XML files.
    ...
    If there are any text contents that contain raw angle brackets like <tag> that are not meant to be XML elements, escape them as &lt;tag&gt;.
    If there are any singleton tags, you should close them or replace them with an equivalent description."""
            user_prompt = f"Fix the following XML file according to the given instructions. Be especially vigilant for unclosed tags:\n{xml_string}\n"
...
def get_yes_no_input(prompt: Optional[str] = None, rich_open: str = "", rich_close: str = "", with_cancel: bool = False) -> Optional[bool]:
    if bool(rich_open) ^ bool(rich_close):
        ...
        print(f"[red][bold]{error_message}[/bold]")
